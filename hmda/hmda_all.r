...
files_hmda <- here::here(str_glue("/hmda/hmda_2014_nationwide_first-lien-owner-occupied-1-4-family-records_labels.csv"))

  # Output file for processed data
file_out_all <- here::here("/hmda/hmda_2014.rds")
...
# read_hmda <- function(file_raw) {
  read_csv(file = files_hmda, col_types = var_cols) %>%
    filter(
      loan_purpose == 1,
      !is.na(msamd)
    ) %>%
